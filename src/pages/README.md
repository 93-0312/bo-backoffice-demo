...
| `dashboard` | `/` | `widgets/stat-card` · `recent-orders` + `shared/Meter` |
| `dashboard-renewal` | `/dashboard-renewal` | `widgets/renewal-*` 6종 벤토 조립 + `entities/dashboard-renewal` (styled-components 순수 SVG 차트) |
| `users` | `/users` | `features/user-filter·user-create·user-delete` + `widgets/data-table` + `entities/user` 표현 |
...
## 핵심 교보재 포인트
- **상태 소유는 page**: `users` 가 필터 params·목록 데이터를 갖고, `user-filter` 는 변경을 emit 만 합니다.
